...
{!! Form::open(['action' => ['App\Http\Controllers\ReportsController@StoreReport', 'method' => 'POST']]) !!}
    ...
    @foreach($clients as $client)
            @if($client->status != -1)
                <tr>
                    ...
                    @if ($loop->first) <th>{!! Form::radio('$reportedClient', $client->id, true) !!}</th>
                    @else <th>{!! Form::radio('$reportedClient', $client->id) !!}</th> @endif
                </tr>
            @endif
        @endforeach
...
{{Form::textarea('description', '', ['placeholder' => 'Powód', 'maxlength' => 255])}}
{{Form::submit('Wyślij')}}
{!! Form::close() !!}
<div class="page">
